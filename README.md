...
4. **Configure the application**
   ...
   The configuration file should be created at `server/config/config.py`. A template file (`server/config/config_template.py`) is provided for reference.
   
   Edit `server/config/config.py` to set:
...
### Backend Configuration (`server/config/config.py`)
...
```bash
python server/dev_toolkit/run_ngrok.py [BACKEND_PORT]
```
...
- The ngrok script is located in `server/dev_toolkit/` along with other development utilities
- When using ngrok, both the frontend and API will be accessible through the same tunnel URL

## Development Notes

### Development Tools (`server/dev_toolkit/`)
...
Auto-generated documentation for backend modules is available in `documentation/autogen/server/`.
